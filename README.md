# project-01-grid
A front-end study project focused on modern layouts and responsiveness using CSS Grid.

# GridMotors

GridMotors is a fictional car showcase created to practice Grid layouts and interface structuring.

## Technologies

- HTML

- CSS (Grid)

## About

This project was built for learning and practice purposes, focusing on page layout, responsive grids, and positioning control using CSS Grid.

## Credits
Design inspirations and images were sourced from Pinterest and used for study purposes only: 
- https://br.pinterest.com/pin/104497653848219100/
- https://br.pinterest.com/pin/78813062224286390/
- https://br.pinterest.com/pin/804525920976572632/
- https://br.pinterest.com/pin/739505201340666935/

Icons:
- GitHub and Pinterest icons from https://icon-icons.com/pt/

## Author

Developed by João Vitor

License

MIT License © 2025 João Vitor
